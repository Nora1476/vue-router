  ...
  <br><br><br>
    <form action="/main" ref="frm" id="frm">
        ...
        <div>
            ...
            {{item}}<input type="checkbox" name="hobby" :id="item" :value="item" v-model="state.hobby">
            </label>
        </div>
        <div>
            <label class="title">Gender</label>
            <label :for="item" v-for="(item, i) in state.genderList" :key="i">
            {{item}}<input type="radio" name="gender" :id="item" :ref="item" :value="item" v-model="state.gender">
            </label>
        </div>
        <div>
...
export default {
    ...
    setup(){
        ...
        function join(){
        if(id.value.value == ''){ 
            alert('아이디를 입력하세요'); id.focus(); 
            return;
            }
        if(name.value.value == ''){
            alert('이름을 입력하세요'); name.focus();
            return;
            }
         if(pass.value.value == ''){ 
            alert('비밀번호를 입력하세요'); pass.focus(); 
            return;
            }
        if(repass.value.value != pass.value.value){
            alert('비밀번호 다름');repass.focus(); 
            return;
            }
        if(m2.value.value.length != 4){
            alert('4자리로 입력하세요');m2.focus(); 
            return;
            }
        if(m3.value.value.length != 4){ 
            alert('4자리로 입력하세요');m3.focus(); 
            return;
            }
    ...
    return{
        state, id, name, pass, repass, m1, m2, m3, 
        title, frm,  cancel, join,
        }
    }
}
</script>
